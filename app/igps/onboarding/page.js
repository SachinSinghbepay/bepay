    ...
    try {
      ...
      if (!currentSenderId) {
        ...
        if (!senderRes.success)
          throw new Error(
            senderRes.message || "Failed to create sender profile",
          );
        ...
        console.log(
          "Skipping Sender Creation (already exists):",
          currentSenderId,
        );
      }
      ...
      if (formData.documents && formData.documents.length > 0) {
        ...
        for (const doc of formData.documents) {
          ...
          const docRes = await igpsService.uploadSenderDocument(
            currentSenderId,
            {
          ...
          if (!docRes.success) {
            ...
            throw new Error(
              `Failed to upload ${doc.fileName}: ${docRes.message}`,
            );
          }
        }
      } else {
      ...
      if (formData.type === "business" && formData.ubos.length > 0) {
        ...
        for (let i = 0; i < formData.ubos.length; i++) {
          ...
          if (!uboRes.success) {
            ...
            throw new Error(
              `Failed to add UBO ${ubo.firstName}: ${uboRes.message}`,
            );
          }
      ...
      if (!verifyRes.success)
        console.warn("Verification trigger warning:", verifyRes.message);
      ...
      saveProgress(formData, currentSenderId, currentStep, createdUboIndices);
      alert(
        `Onboarding failed: ${error.message}. You can try again to resume.`,
      );
    } finally {
      setLoading(false);
    }
        ...
        <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
          ...
          {currentStep === 3 && (
            <StepDocuments
              ...
              onNext={() =>
                formData.type === "business" ? nextStep() : submitOnboarding()
              }
              onBack={prevStep}
