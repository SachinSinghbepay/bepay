...
const SmoothScroll = ({ children }) => {
  ...
  // Improved scroll functions with device-aware durations and better easing
  const getScrollDuration = useCallback((amount) => {
    const baseDuration = deviceType.current === "mobile" ? 1.2 : 1.5;
    return amount > 0.8 ? baseDuration * 1.5 : baseDuration;
  }, []);

  // Enhanced easing function for smoother page scrolling
  const getSmoothEasing = useCallback(() => {
    // Use the same easing as mouse wheel for consistency
    return (t) => Math.min(1, 1.001 - Math.pow(2, -10 * t));
  }, []);
  ...
  const pageScrollUp = useCallback(() => {
    ...
    lenisRef.current.scrollTo(targetScroll, {
      ...
      easing: getSmoothEasing(), // Use same smooth easing as mouse wheel
    });
  }, [getScrollDuration, getSmoothEasing]);
  ...
  const scrollToPosition = useCallback(
    (position) => {
      ...
      lenisRef.current.scrollTo(position, {
        duration: 2.0, // Longer duration for Home/End scrolling
        easing: getSmoothEasing(),
      });
    },
    [getSmoothEasing]
